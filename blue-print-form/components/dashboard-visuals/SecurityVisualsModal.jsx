...
const SpinnerOverlay = ({ message }) => (
    <div className="flex flex-col items-center justify-center py-32 gap-4">
        <div className="w-12 h-12 border-4 border-[var(--nui-accent)] border-t-transparent rounded-full animate-spin" />
        <p className="text-[var(--nui-text-3)] text-sm font-medium">{message || "Loading…"}</p>
    </div>
);
...
const SecurityVisualsModal = ({ onClose }) => {
    ...
    return (
        ...
        >
            {/* Modal panel */}
            <div className="relative flex flex-col w-full h-full max-w-7xl mx-auto my-4 mx-4 bg-[var(--nui-surface-sunk)] rounded-2xl shadow-2xl overflow-hidden">
                {/* Modal header */}
                <div className="flex-shrink-0 bg-gradient-to-r from-[var(--nui-brand)] to-[var(--nui-accent)] text-white px-8 py-5 flex items-center justify-between">
                    <div>
                    ...
                    <button
                        onClick={onClose}
                        className="p-2 rounded-xl bg-[var(--nui-surface)]/15 hover:bg-[var(--nui-surface)]/25 transition-colors"
                ...
                {/* Legend bar */}
                <div className="flex-shrink-0 bg-[var(--nui-surface)] border-b border-[var(--nui-line)] px-8 py-3 flex items-center gap-6 text-xs font-semibold text-[var(--nui-text-2)]">
                    <span className="flex items-center gap-2">
                        <span className="inline-block w-3 h-3 rounded-full bg-[var(--nui-ok)]" />
                        Yes — Implemented
                    </span>
                    <span className="flex items-center gap-2">
                        <span className="inline-block w-3 h-3 rounded-full bg-[var(--nui-risk)]" />
                        No — Not Implemented
                    </span>
                    <span className="flex items-center gap-2">
                        <span className="inline-block w-3 h-3 rounded-full bg-[var(--nui-idle)]" />
                        Not Configured
                    </span>
                </div>
                    ...
                    {error && !loading && (
                        <div className="flex items-center justify-center py-32">
                            <div className="text-center">
                                <p className="text-[var(--nui-risk)] font-semibold mb-2">{error}</p>
                                <button
                                    onClick={onClose}
                                    className="mt-2 px-4 py-2 bg-[var(--nui-accent)] text-white rounded-lg text-sm font-medium"
                                >
                                    Close
                                </button>
                            </div>
